...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['mark_notification_read'])) {
        try {
            ...
            // FIXED: Changed notification_id to id to match your table structure
    ...
    // AJAX: Get new notifications count
    if (isset($_POST['get_notifications_count'])) {
        try {
    ...
    if (isset($_POST['get_notifications'])) {
        try {
            $notifications_query = $conn->prepare("
                SELECT *, COALESCE(target_page, 'dashboard') as target_page 
                FROM notification 
                WHERE cadet_id = ? 
...
try {
    $sent_outside_materials = $conn->query("
        SELECT m.*, c.fname, c.lname 
        FROM materials m 
        LEFT JOIN cadet c ON m.telephone = c.number 
...
try {
    $taken_outside_materials = $conn->query("
        SELECT m.*, c.fname, c.lname 
        FROM materials m 
        LEFT JOIN cadet c ON m.telephone = c.number 
...
// Get statistics - ALL YOUR ORIGINAL COUNTS PRESERVED
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Toast Notification - NEW ADDITION */
        .notification-toast {
            position: fixed;
            top: 100px;
            right: 20px;
...
<body>
    <!-- Toast Notification Container - NEW ADDITION -->
    ...
    <div class="main-container">
        ...
        <div class="content">
            ...
            <!-- Dashboard Page - ALL YOUR ORIGINAL CONTENT PRESERVED -->
            ...
            <!-- My Materials Page - PRESERVED EXACTLY AS BEFORE -->
            ...
            <!-- Checked Out Materials Page - PRESERVED EXACTLY AS BEFORE -->
            ...
            <!-- Available Materials Page - PRESERVED EXACTLY AS BEFORE -->
            ...
            <!-- Sent Outside Materials Page - PRESERVED EXACTLY AS BEFORE -->
            ...
            <!-- Taken Outside Materials Page - PRESERVED EXACTLY AS BEFORE -->
            ...
            <!-- Notifications Page - PRESERVED EXACTLY AS BEFORE -->
            ...
            <!-- Contact Page - PRESERVED EXACTLY AS BEFORE -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(e) {
            ...
            // Handle toast notification clicks
            const toast = e.target.closest('.notification-toast');
            if (toast && toast.classList.contains('show')) {
                const notificationId = toast.dataset.notificationId;
                const targetPage = toast.dataset.targetPage;
                handleNotificationClick(notificationId, targetPage);
                return; // Prevent other click handlers
            }
            
            // Handle view all notifications link
            if (e.target.classList.contains('view-all') || e.target.closest('.view-all')) {
                e.preventDefault();
    ...
    function checkForNewNotifications() {
        fetch('', {
            ...
            body: 'get_notifications_count=1'
        })
    ...
    function fetchNotifications() {
        ...
        .then(data => {
            if (data.success && data.notifications.length > 0) {
                ...
                const latestUnread = data.notifications.find(n => !n.is_read);
                if (latestUnread) {
    ...
    function showNotificationToast(notification) {
        ...
        toast.dataset.targetPage = notification.target_page;
        toast.classList.add('show', 'unread');
    ...
    function updateNotificationDropdown(notifications) {
        ...
        if (notificationBody && document.getElementById('notificationDropdown').classList.contains('show')) {
            let html = '';
                ...
                notifications.forEach(notification => {
                    html += `
                        <div class="notification-item ${notification.is_read ? 'read' : 'unread'}" 
                             ...
                             data-target-page="${escapeHtml(notification.target_page)}">
                            <div class="notification-title">${escapeHtml(notification.title)}</div>
                            <div class="notification-message">${escapeHtml(notification.message)}</div>
                            <div class="notification-time">
                                ${new Date(notification.created_at).toLocaleString()}
                                ${!notification.is_read ? '<span style="color: var(--primary); margin-left: 0.5rem;">• New</span>' : ''}
                            </div>
                        </div>
                    `;
                });
    ...
    function handleNotificationClick(notificationId, targetPage) {
        console.log('Notification clicked:', notificationId, targetPage); // Debug log
